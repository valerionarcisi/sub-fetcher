...
### Key Components
- **SubdlClient**: primary provider, REST API, downloads ZIP archives
- **OSClient**: fallback provider, XML-RPC, `_download_first_valid()` tries up to 5 results skipping VIP placeholders
- **Claude API integration**: translates English subtitles to Italian in batches of 100 blocks
- **ffsubsync integration**: aligns subtitle timecodes to video audio via Voice Activity Detection. Uses `os.system()` (not `subprocess.run` with pipes — pipes interfere with ffsubsync's `rich` library)
- **ffprobe integration**: detects Italian audio tracks to skip Italian-language films
- **Telegram Bot**: user interaction (grouped notifications, batch downloads, progress bar, commands)
- **State management**: JSON file tracking asked/downloaded/failed/excluded state + Claude costs
...
```
Tests use only `unittest` (stdlib). The test harness patches `/config` paths to a temp directory. 37 tests covering parsing, search cascade, grouping, placeholder detection, Subdl client, ffprobe mocking.
...
Free text searches the media library.
